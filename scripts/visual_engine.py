...
Ten archetypes drawn purely from the planned labels/data the visual-curator
agent supplies — every figure is topic-linked and explanatory, never a
random stock image, and no archetype ever fabricates numbers: a numeric
archetype (comparison_bars/time_series/distribution/etc.) should only be
requested by the agent when the underlying data is real.

Usage:
  python3 visual_engine.py render <archetype> <topic.txt> <title.txt> <data.json> <out.svg>
"""
...
def main(argv: list[str]) -> None:
    ...
    from _jsonio import read_json, die

    if len(argv) != 7 or argv[1] != "render":
        die("Usage: python3 visual_engine.py render <archetype> <topic.txt> "
           "<title.txt> <data.json> <out.svg>")
        return
    archetype = argv[2]
    topic = Path(argv[3]).read_text(encoding="utf-8").strip()
    title = Path(argv[4]).read_text(encoding="utf-8").strip()
    data = read_json(argv[5])
    svg = render(archetype, topic, title, data)
    Path(argv[6]).write_text(svg, encoding="utf-8")
